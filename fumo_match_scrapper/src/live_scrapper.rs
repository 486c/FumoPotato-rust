...
pub struct LiveScrapper {
    ...
    // Queues
    waiting_for_end: Vec<i64>,


    // Timers
    last_checked_new_matches: Option<Instant>,
    last_checked_queue: Option<Instant>,
}

static NEW_MATCHES_INTERVAL_SECS: u64 = 30;
...
impl LiveScrapper {
    pub fn new(
        ...
        Self {
            ...
            cancel_token,
            last_id: None,
            waiting_for_end: Vec::with_capacity(10),
            db_sender,
            last_checked_new_matches: None,
            last_checked_queue: None,
        }
    }
    
    /// Worker that process all queues
    async fn queue_worker(&mut self) -> eyre::Result<()> {
        todo!()
    }
    ...
    /*
        ...
        Old at the bottom
    */
    /// Fetch a newly created matches and split it into
    /// separate queues if needed
    async fn live_worker(&mut self) -> eyre::Result<()> {
    }

    pub async fn run(mut scrapper: Self) {
        while !scrapper.cancel_token.is_cancelled() {
            let (need_to_new, need_to_queue) = {
                let need_to_new = if let Some(last_checked_new) = scrapper.last_checked_new_matches {
                    last_checked_new.elapsed() > Duration::from_secs(NEW_MATCHES_INTERVAL_SECS)
                } else {
                    true
                };

                let need_to_queue = if let Some(last_checked_queues) = scrapper.last_checked_queue {
                    last_checked_queues.elapsed() > Duration::from_secs(5)
                } else {
                    true
                };
            ...
            /*
            let _ = scrapper.live_worker().await
                .inspect_err(|e| {
                    println!("Error during live worker run: {e}");
                });
